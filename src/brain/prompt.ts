...
export function buildSystemPrompt(hotel: PromptHotel, session: PromptSession, deptModes?: DeptModeMap, catalogText?: string, pendingText?: string, contextText?: string): string {
  ...
  return [
    ...
    "13. ABOUT THE HOTEL: timings, check-in and check-out, Wi-Fi, breakfast, facilities, parking, policies, directions and what is nearby come only from HOTEL KNOWLEDGE - if it is not there, say you do not have that detail and offer to ask the front desk, never guess. OUTSIDE THE HOTEL: the weather is only what the LOCAL WEATHER line says; news, sport, politics, prices elsewhere and general facts you cannot check - say so and point to the front desk or a reliable app.",
